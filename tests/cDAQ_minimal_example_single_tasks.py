...
class DAQ():
    ...
    def setup_AO(self, channels, waveform, clk_source=""):
        ...
        self._check_error(self.nidaq.DAQmxTaskControl(task['task_handle'], 5))

        return task_name

    def write_AO(self, task_name, waveform):
        task = self.tasklist[task_name]
        if (len(np.shape(waveform)) == 2):
            numChannels = len(waveform)
        else:
    ...
    def stop(self, task_name, clear = False):
    ...
    def set_analog_voltages(self, output_dict):
        ...
        self.write_AO(task_name, voltages)
        self.run(task_name)
        self.waitToFinish(task_name)
        self.stop(task_name, clear = True)
...
def RUN_ME_output_only():
    ...
    for yNum in range(0, len(y_array)):
        ...
        # start counter and scanning sequence
        instruments['daq_out'].write_AO(aotask, x_array)
        instruments['daq_out'].run(aotask)
        instruments['daq_out'].waitToFinish(aotask)
        instruments['daq_out'].stop(aotask)
        instruments['daq_out']._check_error(instruments['daq_out'].nidaq.DAQmxTaskControl(instruments['daq_out'].tasklist[aotask]['task_handle'], 5))
