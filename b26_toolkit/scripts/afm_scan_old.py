...
"""
FG: Script copied from modified version or galco_scan.py
"""



class AFMScanOld(GalvoScanGeneric):
    _DEFAULT_SETTINGS = [
        Parameter('point_a',
        ...
        Parameter('DAQ_channels',
                  # todo: change the channel to the stage and analog in channel I am using
    ...
    def read_line(self, y_pos):
        ...
        #ctrtask = self.daq_in.setup_counter(self.settings['DAQ_channels']['counter_channel'],len(self.x_array) + 1)
        aitask = self.daq_in.setup_AI(channel='ai0',num_samples_to_acquire=len(self.x_array)+1, continuous =True)
        aotask = self.daq_out.setup_AO([self.settings['DAQ_channels']['x_ao_channel']],
                                       self.x_array, clk_source=aitask)
        ...
        xLineData, _ = self.daq_in.read(aitask)
...
if __name__ == '__main__':
    script, failed, instruments = Script.load_and_append(script_dict={'AFMScanOld': 'AFMScanOld'})
