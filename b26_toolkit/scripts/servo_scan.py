...
class SetServo(Script):
    ...
    def transform_coords(self, point):
        ...
        # pt_1_galvo = self.settings['coord_transform']['pt_1']['galvo_coords']
        # pt_2_galvo = self.settings['coord_transform']['pt_2']['galvo_coords']
        # pt_1_servo = self.settings['coord_transform']['pt_1']['servo_coords']
        # pt_2_servo = self.settings['coord_transform']['pt_2']['servo_coords']
        #
        # if self.settings['coord_transform']['use_galvo_coords']:
        #     scale_x = (pt_1_servo['x'] - pt_2_servo['x']) / (pt_1_galvo['x'] - pt_2_galvo['x'])
        #     scale_y = (pt_1_servo['y'] - pt_2_servo['y']) / (pt_1_galvo['y'] - pt_2_galvo['y'])
        #     offset_x = pt_1_servo['x'] - scale_x * pt_1_galvo['x']
        #     offset_y = pt_1_servo['y'] - scale_x * pt_1_galvo['y']
...
class ServoScanFringe(Script):
    ...
    #                             'If true, transform the scan coordinates from galvo voltages to servo displacement. This option allows you to define the scan range using galvo scan voltages, which makes it easier to compare ServoScans and GalvoScans'),
    #                   Parameter('pt_1',
    #                             [Parameter('galvo_coords',
    #                                        [Parameter('x', 0, float, 'position (V) of a calibration point on a GalvoScan'),
    #                                         Parameter('y', 0, float, 'position (V) of a calibration point on a GalvoScan')
    #                                         ]),
    #                              Parameter('servo_coords',
    #                                        [Parameter('x', 0, float, 'position (V) of a calibration point on a ServoScan'),
    #                                         Parameter('y', 0, float, 'position (V) of a calibration point on a ServoScan')
    #                                         ])]),
    #                   Parameter('pt_2',
    #                             [Parameter('galvo_coords',
    #                                        [Parameter('x', 0, float, 'position (V) of a calibration point on a GalvoScan'),
